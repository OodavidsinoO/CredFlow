...
| `GET /editor/scan/templates/{uuid}` | Template detail with `plugins.families` | Returns detail, but **`plugins` key is `None`/absent** — no `plugins.families` | ⚠️ **Changed** (empirical) |
| `GET /editor/scan/{id}` | Source scan config clone | Returns config, but **`plugins` key is `None`/absent** | ⚠️ **Changed** (empirical) |
| `POST /scans` | Create with `uuid` + `settings` + `credentials` + `plugins` | Works with 52-char uuid. **Without X-API-Token → HTTP 412 "API is not available"**; with token → 200. `plugins` payload accepted but **does not take effect** (editor config shows `plugins: None`). | ⚠️ **Changed** (empirical) |
...
### 2. `plugins.families` no longer returned; `plugins` payload ineffective
- **Empirical:** `GET /editor/scan/templates/{uuid}` and `GET /editor/scan/{id}` both return a top-level `plugins` key that is `None` (absent). The `settings` object still contains `basic`/`assessment`/`advanced`/`discovery`/`report` groups, but there is no `plugins.families` map.
- **Empirical:** Passing `"plugins": {"Denial of Service": {"id": 44, "status": "disabled"}}` to `POST /scans` is accepted (HTTP 200) but the resulting scan's editor config shows `plugins: None` — the payload does not take effect.
- **Impact on CredFlow:** **High.** `resolve_disabled_families()` (scanner.py) reads `detail.get("plugins", {}).get("families", {})` and will always get an empty dict, so it returns `{}` and no plugin families are disabled. `extract_plugins_from_config()` similarly returns `{}`. The `plugins` key in `create_scan()` is then omitted. **Behavioral regression** — plugin-family disabling silently no longer works. No official doc describes this change; it is empirical only.
